class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        ### keep only higher time cars,by popping lower time one,
        ## if final result stack is [2,9,19]--- it means 3 fleets wherein [x cars had time<=2,y cars had time<=9,z cars had time<=19]
...
### regardless of faster/slower,the time of car closest to target is first added into stack,
# so any car taking more time than this speed will end up creating a seperate fleet,
# else if theyre taking lesser time,they'll merge with the car in front of it,so pop out time of front car
# pop here means remove prior blocking,new car blockage has arrived.
