...
# dp[3] would be 1 because the longest sub starting from the end of the array would be 1. 
#dp[2] would be 1 if nums[2]>nums[3] and because then it would not be
# increasing and dp[2] would be 1+dp[3] because then we have to include the last element too.
#After that dp[1] would be 1 if nums[1]>nums[2] because then again
# the sub would not be increasing and dp[1] would 1+dp[2] if we include it. 
#After that in the case of dp[0] we will do that thing again and then our final
